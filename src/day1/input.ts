...
export function parsedInput(): Instruction[] {
    const strInstructions = input.split(',').map(s => s.trim());

    return strInstructions.map(strInstruction => {
        const direction = strInstruction[0] as 'L' | 'R';

        const distance = parseInt(strInstruction.slice(1), 10);
